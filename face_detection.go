...
func StartMobilePigo(cas []byte) *MobilePigo {
...
func (mobilego *MobilePigo) GetDetect(bytesImage []byte) {
	...
	dets = mobilego.classifier.ClusterDetections(dets, 0)

	if len(dets) > 0 {
		mobilego.Scale = dets[0].Scale
		mobilego.Row = dets[0].Row
		mobilego.Col = dets[0].Col
		mobilego.Rows = rows
		mobilego.Cols = cols
		mobilego.Q = int(dets[0].Q)
	}
}
...
func StartMobilePigoLandMark(facecascade, puplocCascade, lp38, lp42, lp44, lp46, lp81, lp82, lp84, lp93, lp312 []byte) *MobilePigoLandMark {
	//face classifier
	var __classifier = pigo.NewPigo()
